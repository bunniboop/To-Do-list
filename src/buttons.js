...
function addToProgressBar(e) {
    ...
    if (completedTasks === numOfTasks) {
        ...
        wrapper.querySelectorAll('button').forEach(b => { if (b.getAttribute('toggle') == null) b.disabled = true }); //disable all buttons after completion
...
function filterTasks(e) {
    e.preventDefault()
    const criteria = e.target.getAttribute('sort-data');
    const tasksContainer = e.currentTarget.parentNode.nextElementSibling; //gets task container
    const tasksList = Array.from(tasksContainer.children);
    ...
    tasksList.sort((a, b) => {
        if (criteria === "az") {
            ...
            return a.dataset.addedTime - b.dataset.addedTime;
