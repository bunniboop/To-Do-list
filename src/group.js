...
export function createGroup(e){
    ...
    divGroup.setAttribute("data-added-time", timestamp)
    ...
    const divbar = makeElement({type:'div', className:'progress__div'});
    const progressBAr = html`
<p>0%</p>
<div class='progress'>
    <div class='bar'></div>
</div>
<a class='progress_btn' @click=${clearGroup}>Delete Group</a>`
    render(progressBAr, divbar);
    ...
    divGroup.append(divGroupHeader, divOfTask, divbar)
    ...
    addNew.addEventListener('click', () => addNewTaskToGroup(divOfTask, svgDropMenu, divbar))
}



function addNewTaskToGroup(divOfTask, svgDropMenu, divBar){
    divOfTask.style.display = 'inline-flex';
    divBar.style.display = 'flex';
    changeIcon(svgDropMenu, divOfTask)
...
function clearGroup(e){
    e.currentTarget.closest('.group-task-wrapper').remove()
}
